...
- **Дата и время** каждого круга — два столбца слева, включаются по отдельности.
...
В контекстном меню два независимых пункта — **«Показывать дату»** и **«Показывать время»**.
Включённый столбец добавляется **слева** от номера круга (комментарий при этом остаётся
справа), окно расширяется ровно на ширину столбца и сужается обратно при выключении.

![Столбцы даты и времени](docs/screenshot-columns.png)

Момент фиксируется в тот миг, когда круг записан, и хранится независимо от показа:
можно снять серию с выключенными столбцами, а потом включить их и увидеть время
каждого круга задним числом. Включённые столбцы попадают и в буфер обмена, и в CSV.

Формат задаётся в `config.json` (секция `columns`) строками `strftime` —
по умолчанию `%Y-%m-%d` и `%H:%M:%S`.
...
- **Правый клик** — меню: «✔ Поверх всех окон», «✔ Показывать дату», «✔ Показывать время»,
...
| `columns` | `show_date`, `show_time` — показывать столбцы (переключаются и в меню); `date_format`, `time_format` — строки `strftime`, например `"%d.%m.%Y"` и `"%H:%M"` |
